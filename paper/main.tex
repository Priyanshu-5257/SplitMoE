...
\caption{Three capacity allocations. Standard MoE uses only routed full experts. The DeepSeekMoE-style panel isolates the relevant full shared-expert pattern (omitting its fine-grained segmentation). SplitMoE instead divides active width between partial-width shared and private FFNs. ``Add shared + routed'' denotes the additive combination of the always-active shared output and the router-weighted expert output.}

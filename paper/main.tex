...
\usepackage{url}
\graphicspath{{../results/}{../results/paper_scaling/}{../results/frontier/}{../results/mechanism/}{../results/heldout/}{../results/systems/}}
...
\begin{abstract}
Sparse mixture-of-experts (MoE) layers increase stored capacity without activating every parameter for every token, but independent experts may redundantly learn transformations useful across routes. We study \emph{SplitMoE}, a from-scratch MoE layer that divides active feed-forward width between one always-active shared network and smaller routed private networks. Across five paired seeds in a 46--65M-parameter setting, a 25/75 shared/private allocation reduces MoE-FFN storage by 18.75\% with no detected validation-loss degradation relative to a conventional Top-1 MoE. In an all-MoE, eight-expert Top-1 setting, it reduces total parameters by 16.4\% and improves validation loss by $-0.08640$ with a paired 95\% confidence interval (CI) of $[-0.09483,-0.07798]$; independent LAMBADA evaluation reproduces the direction ($-0.06372$ $[-0.10240,-0.02504]$). The conclusion changes under 16-expert Top-4 routing: a practical Split variant saves 20.1\% total storage at a mean loss cost of $+0.01362$, an equal-active variant closes the observed gap but does not satisfy a preregistered non-inferiority criterion, and a conventional full shared expert achieves the best quality. Post-training interventions show that both Split branches matter and that wrong private experts are worse than omitting the private branch. At matched routed width, Split private outputs have lower centered cosine similarity and linear CKA than conventional experts. The results support partial-width sharing as a regime-dependent parameter-allocation tool, not a universally superior MoE replacement.
\end{abstract}
...
The implementation adds shared and routed outputs and applies a fixed $1/\sqrt{2}$ output scale. Top-1 experiments use straight-through router scaling: forward expert weight is one while router gradients follow the selected probability. Top-4 experiments renormalize selected probabilities. All experiments add a load-balancing auxiliary loss with coefficient 0.01 and router z-loss with coefficient 0.001.

\subsection{Controls}
We distinguish four matching rules. (1) \emph{Equal active width}: Standard-1024 versus Split-25 (256 shared + 768 private) under Top-1. (2) \emph{Equal total storage}: Standard-640 versus Split-50 in the small model. (3) \emph{Practical Top-4}: shared 256 + private 768, reducing storage and activation. (4) \emph{Equal-active Top-4}: shared 256 + four private routes of width 960, since $256+4(960)=4(1024)$. We also compare one always-active width-1024 expert plus 15 routed width-1024 experts with Top-3 routing; this matches the Standard-16E Top-4 FFN storage and activation up to a negligible router-size difference.

\section{Experimental Setup}
Models are decoder-only Transformers with SwiGLU FFNs, RMSNorm, learned positional embeddings, context 256, and the 49,152-token SmolLM2 tokenizer. The main scaling models have eight layers, width 512, eight attention heads, and an MoE in every layer. They train from scratch for 6,500 optimizer steps with an effective batch of 64 sequences (106.5M token positions), AdamW, peak learning rate $3\times10^{-4}$, 500 warmup steps, FP16, and paired seeds 1337, 2027, and 3407.
...
\caption{Five-seed small-model validation quality versus total stored parameters.}
\label{fig:frontier}
\end{figure}

The all-MoE Top-1 comparison strengthens the result (Table~\ref{tab:top1}). Split-25 reduces total storage by 16.4\% at matched activation and wins every paired seed and domain. The paired loss improvement is much larger than in the small model, but these regimes differ in MoE placement and training horizon, so it is not presented as a clean scaling law.

\begin{table}[t]
\centering
\caption{Eight-expert Top-1 all-MoE results (mean over three seeds).}
\label{tab:top1}
\begin{tabular}{lrrr}
\hline
...
\caption{Top-4 quality/storage frontier. Matching activated capacity closes the partial-width Split gap; the full shared expert gives the lowest loss at Standard-like storage.}
\label{fig:top4}
\end{figure}

\subsection{Independent held-out evaluation}
Table~\ref{tab:heldout} reports three-seed means on LAMBADA. The Top-1 result generalizes: Split-25 improves full-token loss in every paired seed. The paired mean difference is $-0.06372$, with 95\% CI $[-0.10240,-0.02504]$. Target-token loss and teacher-forced exact target-sequence rate improve by $-0.34892$ and $+0.01895$; both paired intervals exclude zero. Under Top-4, the full shared expert is numerically better in full-token loss in all seeds, but equal-active Split minus shared is $+0.01019$ $[-0.00762,+0.02801]$ and therefore unresolved.

\begin{table}[t]
\centering
\caption{Independent LAMBADA evaluation (three-seed means). Exact is teacher-forced target-token sequence match.}
\label{tab:heldout}
\begin{tabular}{llrrr}
\hline
...
\caption{Top-1 paired independent evaluation. Each line connects the same training seed.}
\label{fig:heldout}
\end{figure}

\subsection{Causal specialization and redundancy}
...
\caption{Width-controlled expert-output similarity. Split private experts remain less similar than conventional experts with the same width.}
\label{fig:similarity}
\end{figure}

\subsection{Systems behavior}
Training throughput does not track activated parameter count exactly because dispatch and launching separate shared/private FFNs carry overhead. In logged Top-4 training, practical Split is 2.0\% slower than Standard despite fewer activated parameters, and equal-active Split is 3.0\% slower; practical Split reduces peak reserved memory from 6.38 to 5.32 GiB per GPU.
...
\caption{Standardized architecture-only single-T4 benchmark.}
\label{fig:systems}
\end{figure}

\section{Limitations}
The models are small by contemporary language-model standards, use only three confirmatory seeds in the scaling experiments, and train for roughly 106M token positions. The corpus is a deliberately interpretable four-source mixture rather than a modern trillion-token pretraining distribution; its source revisions were not pinned during the original preparation, and it was not deduplicated against the independent evaluation corpus. Top-1 and Top-4 differ in router weighting as well as $K$, so their effect sizes do not isolate $K$ alone. Confidence intervals with three seeds are wide, and failure to reject a difference is not equivalence. The fixed $1/\sqrt{2}$ branch-output scale was not ablated. The reference dispatcher is Python-level and unfused; its throughput should not be extrapolated to optimized expert-parallel kernels. Causal interventions are out of distribution, and similarity uses each model's native hidden states. Prior shared-expert and shared-base/delta work limits our contribution to controlled partial-width allocation evidence.

\section{Conclusion}
SplitMoE asks a simple allocation question: should every routed expert independently store its entire FFN transformation? In the tested Top-1 settings, assigning a modest fraction of active width to a shared path improves the observed quality/storage frontier while preserving routing-dependent, less-redundant private experts; the direction also holds on independently sourced LAMBADA text. Under Top-4 routing, equalizing active capacity closes the partial-width quality gap, but a conventional full shared expert is stronger at matched storage and activation. Shared capacity is useful; the appropriate shared/private granularity depends on routing regime and systems implementation.

\section*{Reproducibility Statement}
The public repository contains model code, exact configurations, pretokenization, paired-seed metrics, intervention and similarity scripts, W\&B exports, and figure-generation scripts. Large checkpoints are not bundled with the Git repository. Training-data sources and preprocessing are specified in configuration; the held-out dataset and tokenizer revisions, preprocessing, and checksums are recorded in a machine-readable manifest. No optimizer state is required for evaluation.

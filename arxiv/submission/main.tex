...
\author{Priyanshu Maurya\\Independent Researcher\\\texttt{mauryac198@gmail.com}}
\date{}
...
\begin{figure}[t]
\centering
\includegraphics[width=.98\linewidth]{architecture.png}
\caption{Expert-bank view of Standard MoE, DeepSeekMoE, and SplitMoE. DeepSeekMoE combines fine-grained expert segmentation with isolated always-active shared experts. SplitMoE does not multiply the expert bank into fine-grained segments; it explicitly divides a target active width into one width-$sD$ shared FFN and width-$(1-s)D$ private FFNs. Orange dashed outlines mark active paths, and $\oplus$ adds the shared output to the router-weighted routed outputs.}
